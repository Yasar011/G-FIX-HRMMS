...
import { saveGarment } from "@/lib/garments";
import { uploadToCloudinary } from "@/lib/cloudinary-upload";
import { ImageSlot } from "./ImageSlot";

type FormState = Omit<Garment, "id" | "createdAt">;
...
export function GarmentForm({
  ...
  const [form, setForm] = useState<FormState>(
    initialGarment ? { ...initialGarment } : EMPTY_FORM
  );
  ...
  async function handleImage(files: FileList | null) {
    ...
    setUploading(true);
    try {
      update("imageUrl", await uploadToCloudinary(file, "garments"));
    } catch {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      await saveGarment(garmentId, {
        ...form,
        createdAt: initialGarment?.createdAt ?? Date.now(),
      });
      router.push("/admin/fashion");
    } catch {
      setError("Could not save the garment.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="flex max-w-xl flex-col gap-5 pb-16">
      <div>
        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Garment image</label>
        {form.imageUrl && (
          <ImageSlot src={form.imageUrl} alt={form.title} className="mt-2 h-48 w-full rounded-lg object-cover" />
        )}
        <input type="file" accept="image/*" onChange={(e) => handleImage(e.target.files)} disabled={uploading} className="mt-2 text-sm" />
      ...
      <div>
        ...
        <textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={3} className={fieldClass()} />
      </div>

      <div>
        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Category</label>
        <input value={form.category} onChange={(e) => update("category", e.target.value)} placeholder="e.g. Casual Shirt, Pattern, Tech Pack" className={fieldClass()} />
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={() => router.push("/admin/fashion")} className="rounded-md border border-zinc-300 px-4 py-2 text-sm dark:border-zinc-700">
          Cancel
        </button>
      </div>
    </div>
